...
const getClosestUpcomingMaintenanceBanner = (maintenances: MaintenanceInfo[]) => {
    ...
    const closestUpcomingMaintenance = maintenances?.filter(maintenance => maintenance.startTime > currentTime).sort((a, b) => a.startTime - b.startTime)[0];
    if (closestUpcomingMaintenance == null) return;
    return (<Tooltip title={closestUpcomingMaintenance.message}><Alert message={`Next upcoming maintenance: ${new Date(closestUpcomingMaintenance.startTime).toLocaleString()} until ${new Date(closestUpcomingMaintenance.endTime).toLocaleString()}`} type="info" closable banner /></Tooltip>);
}

const getCurrentMaintenanceBanner = (maintenances: MaintenanceInfo[]) => {
    const currentTime = Date.now();
    const currentMaintenance = maintenances.find(maintenance => maintenance.startTime < currentTime && maintenance.endTime > currentTime);
    if (currentMaintenance == null) return;
    return (<Tooltip title={currentMaintenance.message}><Alert message={`Currently under maintenance, presumably until ${new Date(currentMaintenance.endTime).toLocaleString()}`} type="warning" closable banner /></Tooltip>);
}
...
export function MaintenanceBanner() {
    ...
    if (maintenances?.length === 0 || maintenances == null) return (<Alert message="No maintenances scheduled or not yet fetched" type="success" closable />); // TODO only for testing, remove me
