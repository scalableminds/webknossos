...
function transpose(vector: Vector3, activeViewport: OrthoView) {
  return Dimensions.transDim(vector, activeViewport);
}

export function* performVolumeInterpolation(
  volumeTracing: VolumeTracing,
  volumeTracingLayer: APISegmentationLayer,
  activeViewport: OrthoView,
  relevantBoxMag1: BoundingBox,
    ...
    interpolationVoxelBuffers[targetOffsetW] = interpolationLayer.createVoxelBuffer2D(
      V2.floor(
        interpolationLayer.globalCoordToMag2DFloat(
          V3.add(relevantBoxMag1.min, transpose([0, 0, targetOffsetW], activeViewport)),
        ),
      ),
      size[firstDim],
      size[secondDim],
...
export default function* maybeInterpolateSegmentationLayer(): Saga<void> {
  ...
  const [_firstDim, _secondDim, thirdDim] = Dimensions.getIndices(activeViewport);
  ...
  const relevantBoxMag1 = viewportBoxMag1
    ...
    .paddedWithSignedMargins(
      transpose(
        [0, 0, -directionFactor * interpolationDepth * labeledResolution[thirdDim]],
        activeViewport,
      ),
    )
