...
class EditableMappingService @Inject() (
    ...
    for {
      _ <- Fox.combined(sourceIterator.map { keyValuePair =>
        for {
          ...
          _ <- tracingDataStore.editableMappingsSegmentToAgglomerate
            .put(newKey, version = newVersion, keyValuePair.value)
        } yield ()
      }.toList)
    } yield ()
  }
  ...
  private def findSegmentIdAtPosition(
    ...
    for {
      ...
      _ <- Fox.bool2Fox(
        voxelAsLongArray.length == 1
      ) ?~> s"Expected one, got ${voxelAsLongArray.length} segment id values for voxel."
      voxelAsLong <- voxelAsLongArray.headOption
    } yield voxelAsLong

  def volumeData(
      editableMappingLayer: EditableMappingLayer,
  ...
  def getAgglomerateSkeletonWithFallback(
      tracingId: String,
      version: Long,
    ...
    for {
      ...
      skeletonBytes <- agglomerateGraphBox match {
        ...
        case Empty =>
          remoteDatastoreClient.getAgglomerateSkeleton(
            remoteFallbackLayer,
  ...
  private def agglomerateGraphToSkeleton(
      tracingId: String,
  ...
  def mapData(
      unmappedData: Array[UnsignedInteger],
      relevantMapping: Map[Long, Long],
      elementClass: ElementClassProto
  ): Fox[Array[Byte]] = {
    val mappedDataLongs: Array[Long] =
      unmappedData.toIndexedSeq.map((element: UnsignedInteger) => relevantMapping(element.toPositiveLong)).toArray
    for {
  ...
  private def longsToBytes(longs: Array[Long], elementClass: ElementClassProto): Fox[Array[Byte]] =
    for {
      _ <- bool2Fox(!elementClass.isuint64)
      unsignedIntArray: Array[UnsignedInteger] = longs.toIndexedSeq
        .map(UnsignedInteger.fromLongWithElementClass(_, elementClass))
        .toArray
  ...
  def agglomerateGraphMinCut(
    ...
    for {
      ...
      agglomerateGraph <- getAgglomerateGraphForIdWithFallback(
        ...
        remoteFallbackLayer
      ) ?~> "getAgglomerateGraph.failed"
      edgesToCut <- minCut(
        agglomerateGraph,
        parameters.segmentId1,
        parameters.segmentId2
      ) ?~> "Could not calculate min-cut on agglomerate graph."
