...
class DataStoreConfig @Inject()(configuration: Configuration) extends ConfigReader {
  ...
  object Datastore {
    ...
    object SingleOrganizationDatastore {
      val enabled: Boolean = get[Boolean]("datastore.singleOrganizationDatastore.enabled")
      val organizationName: String = get[String]("datastore.singleOrganizationDatastore.organizationName")
    }
    object AutoUpdate {
    ...
    val children = List(WebKnossos, WatchFileSystem, Cache, Isosurface, Redis, AgglomerateSkeleton, AutoUpdate)
  }
