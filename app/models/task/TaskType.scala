...
object TaskType extends BasicDAO[TaskType]("taskTypes") {
  ...
  def fromForm(summary: String, description: String, allowedModes: Seq[String], branchPointsAllowed: Boolean, expectedTime: TimeSpan) = 
    TaskType(summary, description, expectedTime, TracingSettings(allowedModes.toList, branchPointsAllowed))
    
  def toForm(tt: TaskType) =
    Some((tt.summary, tt.description, tt.tracingSettings.allowedModes, tt.tracingSettings.branchPointsAllowed, tt.expectedTime))
