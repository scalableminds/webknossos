...
import { Badge, Button, InputNumber, Modal, Table, Tag, Tooltip } from "antd";
...
class ExperienceModalView extends React.PureComponent<Props, State> {
  ...
  sortEntries(entries: Array<TableEntry>): Array<TableEntry> {
    return entries.sort(
      Utils.localeCompareBy([] as Array<TableEntry>, (entry) => entry.domain.toLowerCase()),
    );
  }
  ...
  getTableEntries = (users: Array<APIUser>): Array<TableEntry> => {
    ...
    const allDomains: Array<string> = _.union(
