...
// This module intentionally has no `expose()` call (unlike async_bucket_picker.worker.ts, which
// re-exports `pick` from here and exposes it via Comlink). Comlink's `expose()` defaults its
// message endpoint to `self`, which -- on the main thread -- is `window`. Calling it here would
// mean that simply *importing* this logic for a main-thread (non-worker) call installs a bogus
// Comlink RPC listener on `window`, which then fails trying to postMessage a function reference.
// Keeping `pick` in a plain module lets it be imported directly on the main thread (see
// layer_rendering_manager.ts's useWebWorkerForBucketPicking dev flag) without that side effect.
...
export type ObliquePickerStrategy = "scanLines" | "floodFill" | "wasm" | "floodFillWasm";
