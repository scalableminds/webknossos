...
const vec3 datasetScaleFactor = <%= formatVector3AsVec3(datasetScaleFactor) %>;
  ...
  if (index.x >= 1. && index.x <= PLANE_SUBDIVISION - 1.) {
    transWorldCoord.x =
      (
        ...
        floor(worldCoordTopLeft.x / datasetScaleFactorUVW.x / d.x) * d.x
        // Move by index.x buckets to the right.
        + index.x * d.x
      ) * datasetScaleFactorUVW.x;
  ...
  if (index.y >= 1. && index.y <= PLANE_SUBDIVISION - 1.) {
    transWorldCoord.y =
      (
        ...
        floor(worldCoordTopLeft.y / datasetScaleFactorUVW.y / d.y) * d.y
        // Move by index.y buckets to the bottom.
        + index.y * d.y
      ) * datasetScaleFactorUVW.y;
