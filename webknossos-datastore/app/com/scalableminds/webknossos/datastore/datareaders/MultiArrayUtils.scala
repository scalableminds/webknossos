...
object MultiArrayUtils extends LazyLogging {
  ...
  /** Offset describes the displacement between source and target array.<br/> <br/> For example in the case of one
    * dimensional arrays:<br/> <pre> source array initialized { 1, 2, 3, 4, 5, 6, 7, 8, 9 } target array initialized {
    * -1, -1, -1 } </pre><br/> An offset of 3 means that the target arrays will be displayed that way:<br/> <pre> source
    * { 1, 2, 3, 4, 5, 6, 7, 8, 9 } target { 4, 5, 6 } </pre> An offset of -2 means that the target arrays will be
    * displayed that way:<br/> <pre> source { 1, 2, 3, 4, 5, 6, 7, 8, 9 } target { -1, -1, 1 } </pre>
    *
    * @param offset
    *   \- the displacement between source and target
    * @param source
    *   \- the source array
    * @param target
    *   \- the target array
