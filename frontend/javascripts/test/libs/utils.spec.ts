...
describe("Utils", () => {
  it("filterWithSearchQueryAND: simple case", () => {
    ...
    const matchedElements = filterWithSearchQueryAND(collection, ["prop"], "match");
  ...
  it("filterWithSearchQueryAND: complex case", () => {
    ...
    const matchedElements = filterWithSearchQueryAND(
      collection,
      ["prop", (el) => el.prop2],
      "match keyword",
    );
  ...
  it("filterWithSearchQueryAND: deep case different data types", () => {
    ...
    const matchedElements = filterWithSearchQueryAND(collection, ["prop"], "a key");
    expect(matchedElements.length).toBe(1);
  ...
  it("chunkIntoTimeWindows 1/2", async () => {
    ...
    const chunks = chunkIntoTimeWindows(collection, (el) => el.time, 4);
  ...
  it("chunkIntoTimeWindows 2/2", async () => {
    ...
    const chunks = chunkIntoTimeWindows(collection, (el) => el.time, 9);
  ...
  it("chunkDynamically (I)", () => {
    ...
    const batches = chunkDynamically(elements, 10, (el) => el);
    expect(batches).toEqual([[5, 7], [10, 234], [10]]);
  });

  it("chunkDynamically (II)", () => {
    const elements = [5, 7, 10, 234, 10];
    const batches = chunkDynamically(elements, 100, (el) => el);
    expect(batches).toEqual([[5, 7, 10, 234], [10]]);
  });

  it("chunkDynamically (III)", () => {
    const elements = [5, 7, 10, 234, 10];
    const batches = chunkDynamically(elements, 1000, (el) => el);
    expect(batches).toEqual([[5, 7, 10, 234, 10]]);
  });

  it("chunkDynamically (IV)", () => {
    const elements = [5, 7, 10, 234, 10];
    const batches = chunkDynamically(elements, 1, (el) => el);
    expect(batches).toEqual([[5], [7], [10], [234], [10]]);
  });
  ...
  it("encodeBase62", () => {
    const encoded = encodeToBase62(0);
    expect(encoded).toBe("0");
    const encoded2 = encodeToBase62(123);
    expect(encoded2).toBe("1z");
    const encoded3 = encodeToBase62(123456);
    expect(encoded3).toBe("W7E");
    const encoded4 = encodeToBase62(-1);
    expect(encoded4).toBe("z");
    const encoded5 = encodeToBase62(-2);
    expect(encoded5).toBe("y");
    const encoded6 = encodeToBase62(-123456);
