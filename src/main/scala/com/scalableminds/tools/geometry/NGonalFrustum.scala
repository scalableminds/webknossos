...
class NGonalFrustum(n: Int, h:Int, rBase: Int, rTop: Int) {
  val polygons = {
    ...
    var polys = new Polygon(topP.to3D(h,1).reverse) :: Nil  // top gets reveresed to make it counter clockwise
    for(i<-0 until n){
      ...
      val vertices = baseP.vertex(i).to3D(0,1) :: baseP.vertex(i-1).to3D(0,1) :: topP.vertex(i-1).to3D(h,1) :: topP.vertex(i).to3D(h,1) :: Nil
      polys ::= new Polygon(vertices)
    }
    // return the list of polygons, but first prepend the base polygon
    new Polygon(baseP.to3D(0,1)) :: polys
  }
