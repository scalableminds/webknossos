...
import type {
  NewTaskType,
  TaskCreationResponseType
} from "admin/views/task/task_create_bulk_view";
...
export async function getLoggedTimes(
  userID: ?string
): Promise<Array<APITimeIntervalType>> {
  const url = userID != null
    ? `/api/users/${userID}/loggedTime`
    : "/api/user/loggedTime";
...
export async function pauseProject(
  ...
  const project = await Request.receiveJSON(
    `/api/projects/${projectName}/pause`
  );
  return transformProject(project);
}
...
export async function resumeProject(
  ...
  const project = await Request.receiveJSON(
    `/api/projects/${projectName}/resume`
  );
  return transformProject(project);
}
...
export async function updateTask(
  ...
  const updatedTask = await Request.sendJSONReceiveJSON(
    `/api/tasks/${taskId}`,
    {
      method: "PUT",
      data: task
...
export async function getDatasetAccessList(
  datasetName: string
): Promise<Array<APIUserType>> {
...
export async function addDataset(
  datatsetConfig: DatasetConfigType
): Promise<APIAnnotationType> {
...
export async function getTimeTrackingForUser(
  ...
  const timeTrackingData = await Request.receiveJSON(
    `/api/time/user/${userId}?startDate=${startDate.unix() *
      1000}&endDate=${endDate.unix() * 1000}`
  );
...
export async function getProjectProgressReport(
  ...
  const progressData = await Request.receiveJSON(
    `/api/teams/${teamId}/progressOverview`
  );
  assertResponseLimit(progressData);
  return progressData;
}

export async function getOpenTasksReport(
  teamId: string
): Promise<Array<APIOpenTasksReportType>> {
  const openTasksData = await Request.receiveJSON(
    `/api/teams/${teamId}/openTasksOverview`
  );
