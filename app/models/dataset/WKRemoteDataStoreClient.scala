...
class WKRemoteDataStoreClient(dataStore: DataStore, rpc: RPC) extends LazyLogging {
  ...
  def findPositionWithData(dataset: Dataset, dataLayerName: String): Fox[JsObject] =
    ...
    ).addQueryString("token" -> RpcTokenHolder.webknossosToken).getWithJsonResponse[JsObject]
  ...
  def hasSegmentIndexFile(organizationId: String, datasetName: String, layerName: String)(implicit
      ec: ExecutionContext
  ): Fox[Boolean] = {
