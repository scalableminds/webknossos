...
class AnnotationTransactionService @Inject() (
  ...
  private def saveUncommitted(
    ...
    for {
      _ <- Fox.runIf(transactionGroupIndex > 0)(
        Fox.assertTrue(
          uncommittedUpdatesStore.contains(
  ...
  // and commit them all.
  private def commitWithPending(annotationId: String, updateGroup: UpdateActionGroup)(implicit
      ec: ExecutionContext,
      tc: TokenContext
  ): Fox[Long] =
    for {
  ...
  def handleSingleUpdateAction(annotationId: String, currentVersion: Long, updateAction: UpdateAction)(implicit
      ec: ExecutionContext,
      tc: TokenContext
  ): Fox[Long] = {
    val wrapped = List(
  ...
  def handleUpdateGroups(annotationId: String, updateGroups: List[UpdateActionGroup])(implicit
      ec: ExecutionContext,
      tc: TokenContext
  ): Fox[Long] =
  ...
  private def commitUpdates(annotationId: String, updateGroups: List[UpdateActionGroup])(implicit
      ec: ExecutionContext,
      tc: TokenContext
  ): Fox[Long] =
    for {
  ...
  private def applyImmediatelyIfNeeded(annotationId: String, updates: List[UpdateAction], newVersion: Long)(implicit
      ec: ExecutionContext,
      tc: TokenContext
  ): Fox[Unit] =
  ...
  private def handleUpdateGroup(annotationId: String, updateActionGroup: UpdateActionGroup)(implicit
      ec: ExecutionContext,
      tc: TokenContext
  ): Fox[Unit] =
    for {
  ...
  private def reportUpdates(annotationId: String, updateGroups: List[UpdateActionGroup])(implicit
      tc: TokenContext
  ): Fox[Unit] =
    for {
