    ...
    logTime("project composition", Logger.debug) {
      for {
        ...
        merged <- createFromFinishedAnnotations(
          ...
          annotations,
          AnnotationType.CompoundProject) ?~> Messages("project.noAnnotations")
      } yield merged
    }
  }
    ...
    logTime("task composition", Logger.debug) {
      for {
        ...
        merged <- createFromFinishedAnnotations(
          ...
          annotations,
          AnnotationType.CompoundTask) ?~> Messages("task.noAnnotations")
      } yield merged
    }
  }
    ...
    logTime("taskType composition", Logger.debug) {
      for {
        ...
        merged <- createFromFinishedAnnotations(
          ...
          annotations,
          AnnotationType.CompoundTaskType) ?~> Messages("taskType.noAnnotations")
      } yield merged
    }
  }
