...
trait Secured {
   ...
   */
  def IsMemberOf(project: String)(f: => String => Request[AnyContent] => Result) = IsAuthenticated { user => request =>
   ...
   */
  def IsOwnerOf(task: String)(f: => String => Request[AnyContent] => Result) = IsAuthenticated { user => request =>
