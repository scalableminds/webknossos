...
### Added
- Some failed long-running jobs now show more specific error details than just “Failure”. [#9356](https://github.com/scalableminds/webknossos/pull/9356)
- Added support for reading segment statistics from a precomputed file, as generated by voxelytics. [#9772](https://github.com/scalableminds/webknossos/pull/9772)
- Optimized performance for volume annotation save requests. [#9908](https://github.com/scalableminds/webknossos/pull/9908)
- Added an "Apply View Configuration to All Datasets in Folder" entry to the dataset context menu in the dashboard. It copies the view configuration of the selected dataset to all other datasets in the same folder. Layer settings are only copied to layers of the same name and category. [#9911](https://github.com/scalableminds/webknossos/pull/9911)
- Added support for neuroglancerPrecomputed datasets opened from the python libs with access_mode PROXY_PATH. [#9978](https://github.com/scalableminds/webknossos/pull/9978)
- Added the annotation’s bounding box count to the statistics section in the sidebar info tab. [#9979](https://github.com/scalableminds/webknossos/pull/9979)

### Changed
- While simultaneous editing (live collaboration) is enabled for an annotation, actions that change its set of layers (adding, deleting or converting an annotation layer, merging a volume layer with its fallback layer, importing a volume tracing) are now disabled in the UI with an explanation, instead of potentially causing conflicts. Restoring an older version of an annotation while simultaneous editing is enabled now asks for confirmation and reloads the page for all collaborators, including the user who triggered the restore. [#9917](https://github.com/scalableminds/webknossos/pull/9917)
- Zarr links generated by the frontend now include `/v15/`, inserting the current API version from the buildinfo for better compatibility across WEBKNOSSOS versions. [#9919](https://github.com/scalableminds/webknossos/pull/9919)
- The default zarr streaming routes now use zarr3 instead of zarr2. Zarr2 routes remain supported for a grace period. [#9929](https://github.com/scalableminds/webknossos/pull/9929)
- Shared annotations are no longer listed to users who cannot access the underlying dataset. [#9965](https://github.com/scalableminds/webknossos/pull/9965)
- New HTTP API version 16: GET /datasets no longer accepts the compact param; the response is now always in the compact format. [#9980](https://github.com/scalableminds/webknossos/pull/9980)
- Upgraded react-router from v6 to v7. The deprecated `react-router-dom` package was replaced by `react-router`. [#9988](https://github.com/scalableminds/webknossos/pull/9988)

### Fixed
- Fixed screenshot downloading for Safari which could not handle multi files bulk downloads. Now each download has a little delay inbetween. [#10018](https://github.com/scalableminds/webknossos/pull/10018)
- Fixed that mappings for uint64 segmentation layers could fail with "Cannot rehash, since 20 attempts were exceeded", because 64-bit segment ids were hashed with a poorly distributed hash function. [#10020](https://github.com/scalableminds/webknossos/pull/10020)
- Fixed that the proofreading multi cut tool highlighting was not properly preserved in live collaboration mode when another user does interfering proofreading actions. [#9855](https://github.com/scalableminds/webknossos/pull/9855)
...
- Fixed importing neuroglancers from raw URIs that contain multiple `#` characters (not strictly valid URIs) [#9894](https://github.com/scalableminds/webknossos/pull/9894)
- Scrolling through slices with the mouse wheel is now clamped to the dataset's bounding box, so it's harder to accidentally scroll away from small datasets. Panning and keyboard-based movement are still unrestricted. [#9901](https://github.com/scalableminds/webknossos/pull/9901)
- Fixed reading N5 datasets with compression type `raw`. [#9906](https://github.com/scalableminds/webknossos/pull/9906)
- Fixed adding remote S3 datasets that are addressed in virtual-hosted style (e.g. `https://my-bucket.s3.us-west-2.amazonaws.com/key`). [#9906](https://github.com/scalableminds/webknossos/pull/9906)
- Fixed reading S3 datasets from a custom endpoint whose host name contains no dot but states a port, e.g. `s3://localhost:9000/my-bucket/key` or `s3://minio:9000/my-bucket/key`. [#9906](https://github.com/scalableminds/webknossos/pull/9906)
- Fixed that directory listings for S3 datasets on a custom endpoint could ignore the trailing slash of the listing prefix. [#9906](https://github.com/scalableminds/webknossos/pull/9906)
- Zarr v3 datasets that use a codec WEBKNOSSOS cannot read now report an error naming the codec, instead of failing downstream. [#9906](https://github.com/scalableminds/webknossos/pull/9906)
- Fixed several update actions (e.g., editing a layer's name or the annotation's description, adding a segment index, or another collaborator changing the active mapping) not being incorporated correctly while simultaneous editing (live collaboration) is enabled, which could previously lead to the annotation getting out of sync and requiring a reload. [#9917](https://github.com/scalableminds/webknossos/pull/9917)
...
- Fixed a crash on the Voxelytics workflow report page that occurred when the browser window lost focus while a key was held down. [#9924](https://github.com/scalableminds/webknossos/pull/9924)
- Fixed a bug where sometimes the wrong slice would be rendered in the viewports while flight mode or rotations in ortho view were turned off. [#9926](https://github.com/scalableminds/webknossos/pull/9926)
- Fixed volume interpolation for non-uint64 volume tracings. [#9928](https://github.com/scalableminds/webknossos/pull/9928)
- Fixed off-by-one error when scrolling to the last section (in XY, XZ or YZ) of a dataset. [#9936](https://github.com/scalableminds/webknossos/pull/9936)
- Fixed a bug where server component outages during annotation saving could lead to silently incompletely saved annotations. Now this fails loudly instead. [#9937](https://github.com/scalableminds/webknossos/pull/9937)
- Fixed a bug where some annotation save request retries would wrongly be answered with 409 CONFLICT, forcing a reload. [#9944](https://github.com/scalableminds/webknossos/pull/9944)
- Fixed merger mode for non-64-bit based segmentation layer annotations. [#9955](https://github.com/scalableminds/webknossos/pull/9955)
- Fixed edge case in live collab mode for non-64-bit-based segmentation layers [#9960](https://github.com/scalableminds/webknossos/pull/9960)
- Fixed a rare case where annotation save requests could be partially successful, creating inconsistent state on the server. [#9961](https://github.com/scalableminds/webknossos/pull/9961)
- Fixed updating the largestSegmentId of segmentation layers on the dataset settings page. [#9977](https://github.com/scalableminds/webknossos/pull/9977)

### Breaking Changes
- The backend HTTP API versions v5 through v9 have been removed. v10 (August 2025) is now the oldest supported version. If you still call old API routes, consider using a current release of the [webknossos python library](https://github.com/scalableminds/webknossos-libs) instead. [#9903](https://github.com/scalableminds/webknossos/pull/9903)
