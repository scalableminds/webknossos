...
/** Used to store a mapping from annotation id to datasource and datasetId. This makes it possible for WK to answer a
  * /tracingstores/:name/dataSource or /tracingstores/:name/datasetId request before an annotation is created. This
  * happens when uploading an annotation. It also provides a mapping from temporary/compound annotation id (e.g.
  * taskTypeId, projectId) to datasource.
  */
class AnnotationDataSourceTemporaryStore @Inject() (
  ...
  def store(annotationId: ObjectId, dataSource: UsableDataSource, datasetId: ObjectId)(implicit
      ec: ExecutionContext
  ): Unit =
