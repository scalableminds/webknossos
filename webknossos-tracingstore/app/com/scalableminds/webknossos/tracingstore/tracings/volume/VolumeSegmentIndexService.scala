...
object VolumeSegmentIndexService {
  def canHaveSegmentIndex(
      remoteDatastoreClient: TSRemoteDatastoreClient,
      fallbackLayer: Option[RemoteFallbackLayer]
  )(implicit ec: ExecutionContext, tc: TokenContext): Fox[Boolean] =
    fallbackLayer match {
...
class VolumeSegmentIndexService @Inject() (
  ...
  def updateFromBucket(
    ...
    for {
      ...
      previousBucketBytesWithEmptyFallback <- bytesWithEmptyFallback(
        previousBucketBytesBox,
        elementClass
      ) ?~> "volumeSegmentIndex.update.getPreviousBucket.failed"
      ...
      previousSegmentIds: Set[Long] <- collectSegmentIds(
        ...
        elementClass
      ) ?~> "volumeSegmentIndex.update.collectSegmentIds.failed"
      ...
      ) ?~> "volumeSegmentIndex.update.removeBucket.failed"
      ...
      ) ?~> "volumeSegmentIndex.update.addBucket.failed"
    } yield ()

  private def bytesWithEmptyFallback(bytesBox: Box[Array[Byte]], elementClass: ElementClassProto)(implicit
      ec: ExecutionContext
  ): Fox[Array[Byte]] =
    bytesBox match {
  ...
  private def addBucketToSegmentIndex(
    ...
    for {
      previousBuckets <- segmentIndexBuffer.getSegmentToBucketIndexMap(
        segmentIds,
        bucketPosition.mag,
        mappingName,
        editableMappingTracingId,
  ...
  private def collectSegmentIds(bytes: Array[Byte], elementClass: ElementClassProto)(implicit
      ec: ExecutionContext
  ): Fox[Set[Long]] =
    for {
      set <- tryo(UnsignedIntegerArray.toSetFromByteArray(bytes, elementClass)).toFox
    } yield set.filter(!_.isZero).map { (u: UnsignedInteger) =>
      u.toPositiveLong
    }
  ...
  )(implicit ec: ExecutionContext, tc: TokenContext): Fox[ListOfVec3IntProto] =
    for {
      bucketListBox <- getSegmentToBucketIndex(
        fallbackLayer,
        tracingId,
        segmentId,
        mag,
  ...
  private def addEmptyFallback(
      positionsBox: Box[ListOfVec3IntProto]
  )(implicit ec: ExecutionContext): Fox[ListOfVec3IntProto] =
    positionsBox match {
  ...
  private def getSegmentToBucketIndex(
    ...
    for {
      fromMutableIndex <- getSegmentToBucketIndexFromFossilDB(
        tracingId,
        segmentId,
        mag,
        version,
      ...
      fromFileIndex <- fallbackLayerOpt match { // isEmpty is not the same as length == 0 here :(
        case Some(fallbackLayer) if fromMutableIndex.length == 0 =>
          getSegmentToBucketIndexFromFile(
            fallbackLayer,
            segmentId,
            mag,
            mappingName,
            editableMappingTracingId
          ) // additional coordinates not supported, see #7556
  ...
  private def getSegmentToBucketIndexFromFile(
      layer: RemoteFallbackLayer,
      segmentId: Long,
      mag: Vec3Int,
      mappingName: Option[String],
      editableMappingTracingId: Option[String]
