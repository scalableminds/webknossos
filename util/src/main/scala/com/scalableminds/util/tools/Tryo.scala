...
// https://github.com/lift/framework/blob/main/core/common/src/main/scala/net/liftweb/common/Tryo.scala

trait Tryo {

  /** Wraps a "try" block around the function f. If f throws an exception with its class in the 'ignore' list or if
    * 'ignore' is null or an empty list, ignore the exception and return None.
    *
    * @param ignore
    *   \- a list of exception classes to ignore. A thrown exception will be ignored if it is assignable from one of the
    *   exception classes in the list
    * @param onError
    *   \- an optional callback function that will use the thrown exception as a parameter
    * @param f
    *   \- the block of code to evaluate
    * @return
    *   <ul> <li>Full(result of the evaluation of f) if f doesn't throw any exception <li>a Failure if f throws an
    *   exception <li>Empty if the exception class is in the ignore list </ul>
    */
  ...
  /** Wraps a "try" block around the function f. If f throws an exception that is in the domain of the handler PF, the
    * handler will be invoked on the exception. Otherwise the exception is wrapped into a Failure.
    *
    * @param handler
    ...
    *   exception </ul>
    * @see
    *   com.scalableminds.util.tools.Failure
    */
