    ...
    UsingFilters(
      ...
      Filter("isActive", (value: Boolean, el: DataSet) =>
        ...
        for {
          ...
          requestingUserTeamMemberships <- timed(Fox.runOptional(request.identity)(user => userService.teamManagerMembershipsFor(user._id)), "teamMemberships")
          js <- timed(Fox.serialCombined(filtered)(d => dataSetService.publicWrites(d, request.identity, skipResolutions = true, requestingUserTeamMemberships)), "write")
        } yield {
          Ok(Json.toJson(js))
        }
    }
