...
/**
  *
  * This is a copy of the ZarrStreamingController using datasetId instead of organizationId/datasetDirectoryName
  */
class WKZarrStreamingController @Inject()(
  ...
  def requestDataSource(
    ...
    accessTokenService.validateAccessFromTokenContext(UserAccessRequest.readDataset(datasetId)) {
      for {
        datasetIdValidated <- ObjectId.fromString(datasetId)
        dataSource <- datasetCache.getById(datasetIdValidated) ~> NOT_FOUND
  ...
  def dataSourceWithAnnotationPrivateLink(accessToken: String, zarrVersion: Int): Action[AnyContent] =
    Action.async { implicit request =>
      for {
        ...
        dataSource <- datasetCache.getById(annotationSource.datasetId) ~> NOT_FOUND
        ...
        ifIsAnnotationLayer = (annotationLayer, annotationSource, relevantTokenContext) =>
          remoteTracingstoreClient
            .getRawZarrCube(annotationLayer.tracingId, mag, coordinates, annotationSource.tracingStoreUrl)(
              relevantTokenContext)
            .map(Ok(_)),
        orElse = annotationSource => rawZarrCube(annotationSource.datasetId.toString, dataLayerName, mag, coordinates)
      )
    }
  ...
  private def rawZarrCube(
    ...
    for {
      datasetIdValidated <- ObjectId.fromString(datasetId)
      (dataSource, dataLayer) <- datasetCache.getWithLayer(datasetIdValidated, dataLayerName) ~> SERVICE_UNAVAILABLE
  ...
  private def zArray(datasetId: String, dataLayerName: String, mag: String)(implicit m: MessagesProvider): Fox[Result] =
    for {
      datasetIdValidated <- ObjectId.fromString(datasetId)
      (_, dataLayer) <- datasetCache.getWithLayer(datasetIdValidated, dataLayerName) ?~> Messages("dataSource.notFound") ~> NOT_FOUND
      magParsed <- Vec3Int
        .fromMagLiteral(mag, allowScalar = true)
        .toFox ?~> Messages("dataLayer.invalidMag", mag) ~> NOT_FOUND
      _ <- Fox.fromBool(dataLayer.containsMag(magParsed)) ?~> Messages("dataLayer.wrongMag", dataLayerName, mag) ~> NOT_FOUND
  ...
  private def zarrJsonForMag(datasetId: String, dataLayerName: String, mag: String)(
  ...
  def zArrayPrivateLink(accessToken: String, dataLayerName: String, mag: String): Action[AnyContent] = Action.async {
    implicit request =>
      ifIsAnnotationLayerOrElse(
        ...
        orElse = annotationSource => zArray(annotationSource.datasetId.toString, dataLayerName, mag)
      )
  }
      ...
      ifIsAnnotationLayerOrElse(
        ...
        ifIsAnnotationLayer = (annotationLayer, annotationSource, relevantTokenContext) =>
          remoteTracingstoreClient
            ...
            .map(z => Ok(Json.toJson(z))),
        orElse = annotationSource => zarrJsonForMag(annotationSource.datasetId.toString, dataLayerName, mag)
      )
  }
  ...
  def requestDataLayerMagDirectoryContents(datasetId: String,
                                           dataLayerName: String,
                                           mag: String,
  ...
  private def dataLayerMagDirectoryContents(datasetId: String, dataLayerName: String, mag: String, zarrVersion: Int)(
      implicit m: MessagesProvider): Fox[Result] =
    for {
      datasetIdValidated <- ObjectId.fromString(datasetId)
      (_, dataLayer) <- datasetCache.getWithLayer(datasetIdValidated, dataLayerName) ~> NOT_FOUND
      magParsed <- Vec3Int
        .fromMagLiteral(mag, allowScalar = true)
        .toFox ?~> Messages("dataLayer.invalidMag", mag) ~> NOT_FOUND
      _ <- Fox.fromBool(dataLayer.containsMag(magParsed)) ?~> Messages("dataLayer.wrongMag", dataLayerName, mag) ~> NOT_FOUND
  ...
  def dataLayerMagDirectoryContentsPrivateLink(accessToken: String,
    ...
    Action.async { implicit request =>
      ifIsAnnotationLayerOrElse(
        ...
        orElse = annotationSource =>
          dataLayerMagDirectoryContents(annotationSource.datasetId.toString, dataLayerName, mag, zarrVersion)
      )
    }

  def requestDataLayerDirectoryContents(datasetId: String,
                                        dataLayerName: String,
  ...
  private def dataLayerDirectoryContents(datasetId: String, dataLayerName: String, zarrVersion: Int)(
      implicit m: MessagesProvider): Fox[Result] =
    for {
      datasetIdValidated <- ObjectId.fromString(datasetId)
      (_, dataLayer) <- datasetCache.getWithLayer(datasetIdValidated, dataLayerName) ?~> Messages("dataSource.notFound") ~> NOT_FOUND
  ...
  def dataLayerDirectoryContentsPrivateLink(accessToken: String,
    ...
    Action.async { implicit request =>
      ifIsAnnotationLayerOrElse(
        ...
        orElse = annotationSource =>
          dataLayerDirectoryContents(annotationSource.datasetId.toString, dataLayerName, zarrVersion)
      )
    }

  def requestDataSourceDirectoryContents(datasetId: String, zarrVersion: Int): Action[AnyContent] =
    Action.async { implicit request =>
      accessTokenService.validateAccessFromTokenContext(UserAccessRequest.readDataset(datasetId)) {
        for {
          datasetIdValidated <- ObjectId.fromString(datasetId)
          dataSource <- datasetCache.getById(datasetIdValidated) ?~> Messages("dataSource.notFound") ~> NOT_FOUND
  ...
  def requestZGroup(datasetId: String, dataLayerName: String = ""): Action[AnyContent] =
    Action.async { implicit request =>
