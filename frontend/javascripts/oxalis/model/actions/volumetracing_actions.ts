...
type ResetContourAction = {
  type: "RESET_CONTOUR";
};
export type FinishAnnotationStrokeAction = {
  type: "FINISH_ANNOTATION_STROKE";
  tracingId: string;
};
type SetMousePositionAction = {
  type: "SET_MOUSE_POSITION";
  position: Vector2 | null | undefined;
};
type HideBrushAction = {
  type: "HIDE_BRUSH";
};
type SetContourTracingModeAction = {
  type: "SET_CONTOUR_TRACING_MODE";
  mode: ContourMode;
};
export type ImportVolumeTracingAction = {
  type: "IMPORT_VOLUMETRACING";
};
export type SetMaxCellAction = {
  type: "SET_MAX_CELL";
  cellId: number;
};
...
export const setMousePositionAction = (
  position: Vector2 | null | undefined,
): SetMousePositionAction => ({
  type: "SET_MOUSE_POSITION",
  position,
