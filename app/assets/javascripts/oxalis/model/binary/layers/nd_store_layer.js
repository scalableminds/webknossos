  ...
  requestFromStoreImpl(batch, token) {
    ...
    return Request.receiveArraybuffer(url).then(
      responseBuffer => {
        ...
        let index = 0;
        for (let z of __range__(bucketBounds[2], bucketBounds[5], false)) {
          for (let y of __range__(bucketBounds[1], bucketBounds[4], false)) {
            for (let x of __range__(bucketBounds[0], bucketBounds[3], false)) {
