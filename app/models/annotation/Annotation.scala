...
class AnnotationDAO @Inject()(sqlClient: SQLClient)(implicit ec: ExecutionContext)
  ...
  def countAllForOrganization(organizationId: ObjectId)(implicit ctx: DBAccessContext): Fox[Int] =
    for {
      countList <- run(sql"select count(*) from (select a._id from #${existingCollectionName} a join webknossos.users_ u on a._user = u._id where u._organization = ${organizationId}) q".as[Int])
      count <- countList.headOption
    } yield count
