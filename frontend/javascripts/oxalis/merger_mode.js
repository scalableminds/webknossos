...
import { getSegmentationAnnotationLayer } from "oxalis/model/accessors/dataset_accessor";
  ...
  unsubscribeFunctions.push(
    Store.subscribe(() => {
      ...
      getSkeletonTracing(state.tracing).map(skeletonTracing => {
        const segmentationLayer = getSegmentationAnnotationLayer(state);
...
export function disableMergerMode() {
  ...
  const segmentationLayer = getSegmentationAnnotationLayer(Store.getState());
  if (segmentationLayer != null) {
    api.data.setMappingEnabled(false, segmentationLayer.name);
  }
}
