...
then
  ensureUpToDateTests
  export NODE_PATH="$testBundlePath" && BABEL_ENV=test nyc --silent --no-clean --exclude binaryData ava $(find "$testBundlePath" -name "shader_syntax.spec.js") "$@"
elif [ $cmd == "test-debug" ]
then
  export NODE_PATH="$testBundlePath" && BABEL_ENV=test ava debug $(find "$testBundlePath" -name "shader_syntax.spec.js") "$@"
elif [ $cmd == "test-e2e" ]
then
  ensureUpToDateTests
