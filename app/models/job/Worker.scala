...
class WorkerDAO @Inject()(sqlClient: SqlClient)(implicit ec: ExecutionContext)
  ...
  def findOneByKey(key: String): Fox[Worker] =
    for {
      r: Seq[WorkersRow] <- run(
        sql"select #${columns.debugInfo} from #${existingCollectionName.debugInfo} where key = $key".as[WorkersRow])
  ...
  def findOneByDataStore(dataStoreName: String): Fox[Worker] =
    for {
      r: Seq[WorkersRow] <- run(
        sql"select #${columns.debugInfo} from #${existingCollectionName.debugInfo} where _dataStore = $dataStoreName"
          .as[WorkersRow])
