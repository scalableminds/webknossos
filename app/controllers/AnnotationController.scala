...
class AnnotationController @Inject() (
    ...
    sil.SecuredAction.async { implicit request =>
      for {
        ...
        annotationB <- provider.provideAnnotation(
          ...
          mergedId,
          request.identity
        ) ?~> "annotation.notFound" ~> NOT_FOUND
        mergedAnnotation <- annotationMerger.mergeTwo(
          annotationA,
          annotationB,
          request.identity
        ) ?~> "annotation.merge.failed"
  ...
  def editLockedState(typ: String, id: ObjectId, isLockedByOwner: Boolean): Action[AnyContent] =
    sil.SecuredAction.async { implicit request =>
      for {
        ...
        _ <- bool2Fox(
          annotation.typ == AnnotationType.Explorational
        ) ?~> "annotation.isLockedByOwner.explorationalsOnly"
        _ = logger.info(
  ...
  private def finishAnnotation(typ: String, id: ObjectId, issuingUser: User, timestamp: Instant)(implicit
      ctx: DBAccessContext
  ...
  def editAnnotation(typ: String, id: ObjectId): Action[JsValue] = sil.SecuredAction.async(parse.json) {
    implicit request =>
      for {
        ...
        _ <-
          if (visibility.contains(AnnotationVisibility.Private))
        ...
        _ <- Fox.runOptional(description)(
          annotationDAO.updateDescription(annotation._id, _)
        ) ?~> "annotation.edit.failed"
        ...
        _ <- Fox.runOptional(viewConfiguration)(vc =>
          annotationDAO.updateViewConfiguration(annotation._id, Some(vc))
        ) ?~> "annotation.edit.failed"
  ...
  def listExplorationals(
    ...
    sil.SecuredAction.async { implicit request =>
      for {
        ...
        ) ?~> "annotation.countReadable.failed"
  ...
  def updateOthersMayEdit(typ: String, id: ObjectId, othersMayEdit: Boolean): Action[AnyContent] =
    sil.SecuredAction.async { implicit request =>
      for {
        ...
        ) ?~> "annotation.othersMayEdit.onlyExplorationalOrTask"
        _ <- bool2Fox(annotation._user == request.identity._id) ?~> "notAllowed" ~> FORBIDDEN
        _ <- annotationDAO.updateOthersMayEdit(annotation._id, othersMayEdit)
      } yield Ok(Json.toJson(othersMayEdit))
    }

  private def duplicateAnnotation(annotation: Annotation, user: User)(implicit
      ctx: DBAccessContext,
      m: MessagesProvider
  ): Fox[Annotation] =
    for {
      ...
      dataset <- datasetDAO.findOne(annotation._dataset)(
        GlobalAccessContext
      ) ?~> "dataset.notFoundForAnnotation" ~> NOT_FOUND
      ...
      clonedAnnotation <- annotationService.createFrom(
        ...
        newAnnotationId
      ) ?~> Messages("annotation.create.failed")
