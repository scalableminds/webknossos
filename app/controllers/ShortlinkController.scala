...
class ShortlinkController @Inject()(
  ...
  def create: Action[String] = sil.SecuredAction.async(validateJson[String]) {
    implicit request =>
      val longLink = request.body
      val _id = ObjectId.generate
      val shortLink = RandomIDGenerator.generateBlocking(12)
      for {
        _ <- shortLinkDAO.insertOne(
          ShortLink(_id, shortLink, longLink)) ?~> "create.failed"
        inserted <- shortLinkDAO.findOne(_id)
        js <- shortLinkService.toJson(inserted)
      } yield Ok(js)
  }

  def get(id: String): Action[AnyContent] = sil.SecuredAction.async {
    implicit request =>
      for {
  ...
  def get_by_value(shortLink: String): Action[AnyContent] = Action.async {
    implicit request =>
      for {
