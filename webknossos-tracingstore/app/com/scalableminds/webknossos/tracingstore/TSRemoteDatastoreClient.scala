...
class TSRemoteDatastoreClient @Inject()(
  ...
  private def getRemoteLayerUri(remoteLayer: RemoteFallbackLayer): Fox[String] =
    for {
      datastoreUri <- dataStoreUriWithCache(remoteLayer.organizationId, remoteLayer.datasetPath)
    } yield
      s"$datastoreUri/data/datasets/${remoteLayer.organizationId}/${remoteLayer.datasetPath}/layers/${remoteLayer.layerName}"

  private def dataStoreUriWithCache(organizationId: String, datasetPath: String): Fox[String] =
    dataStoreUriCache.getOrLoad(
      (organizationId, datasetPath),
