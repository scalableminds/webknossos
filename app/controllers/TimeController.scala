...
class TimeController @Inject() (
  ...
  def userLoggedTime(userId: ObjectId): Action[AnyContent] =
    sil.SecuredAction.fox { implicit request =>
      for {
        ...
        timesGrouped: Map[Month, Duration] = timeSpanService.sumTimespansPerInterval(
        ...
        Json.obj(
          "loggedTime" ->
            timesGroupedSorted.map { case (paymentInterval, duration) =>
  ...
  def timeSummedByAnnotationForUser(
      userId: ObjectId,
      start: Long,
      end: Long,
    ...
    for {
      ...
      annotationTypesValidated <- AnnotationType.fromCommaSeparated(
        annotationTypes
      ) ?~> Msg.TimeTracking.invalidAnnotationType
      annotationStatesValidated <- AnnotationState.fromCommaSeparated(
        annotationStates
      ) ?~> Msg.TimeTracking.invalidAnnotationState
      ...
      timesByAnnotation <- timeSpanDAO.summedByAnnotationForUser(
        user._id,
        Instant(start),
        Instant(end),
  ...
  def timeSpansOfUser(
    ...
    sil.SecuredAction.fox { implicit request =>
      for {
        ...
        isTeamManagerOrAdmin <- userService.isTeamManagerOrAdminOf(request.identity, user)
        _ <- Fox.fromBool(isTeamManagerOrAdmin || user._id == request.identity._id) ?~> Msg.notAllowed ~> FORBIDDEN
        timeSpansJs <- timeSpanDAO.findAllByUserWithTask(
          user._id,
          Instant(start),
          Instant(end),
          annotationTypesValidated,
  ...
  def timeOverview(
    ...
    sil.SecuredAction.fox { implicit request =>
      for {
        ...
        _ <- Fox.fromBool(
          annotationTypesValidated.forall(typ => typ == AnnotationType.Explorational || typ == AnnotationType.Task)
        ) ?~> Msg.TimeTracking.unsupportedAnnotationType
        projectIdsValidated <- ObjectId.fromCommaSeparated(projectIds)
        ...
        usersWithTimesJs <- timeSpanDAO.timeOverview(
          Instant(start),
          Instant(end),
