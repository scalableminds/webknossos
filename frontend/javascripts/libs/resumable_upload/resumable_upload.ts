...
import type { ResumableChunk } from "./resumable_chunk";
import { ResumableFile } from "./resumable_file";
import { formatSize, generateUniqueIdentifier } from "./resumable_shared";
...
export class ResumableUpload implements EventTarget {
  ...
  constructor(opts: Partial<ConfigurationHash> = {}) {
    ...
    this.defaults = {
      ...
      minFileSizeErrorCallback: (file: File) => {
        console.error(
          `${file.name} is too small, please upload files larger than ${formatSize(this.getOpt("minFileSize"))}.`,
        );
      },
      maxFileSize: Number.POSITIVE_INFINITY,
      maxFileSizeErrorCallback: (file: File) => {
        console.error(
          `${file.name} is too large, please upload files less than ${formatSize(this.getOpt("maxFileSize"))}.`,
        );
      },
      fileType: [],
  ...
  private appendFilesFromFileList(fileList: File[], event: Event): void {
    ...
    for (const file of fileList) {
      ...
      const uniqueIdentifier = generateUniqueIdentifier(file, event);
