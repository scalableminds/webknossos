...
describe("DebouncedAbortableSaga", () => {
  it("Test simplest case", async () => {
    ...
    await sleep(2000);
    expect(protocol).toEqual(["await-1", "run-1"]);
  });

  it("Rapid calls where the last one should win", async () => {
    const { abortableFn, protocol } = createAbortableFnWithProtocol();
    const fn = createDebouncedAbortableCallable(abortableFn, DEBOUNCE_THRESHOLD, this);
    fn({ id: 1 });
    fn({ id: 2 });
    ...
    await sleep(2000);
    expect(protocol).toEqual(["await-3", "run-3"]);
  });

  it("Rapid calls with small breaks", async () => {
    const { abortableFn, protocol } = createAbortableFnWithProtocol();
    const fn = createDebouncedAbortableCallable(abortableFn, DEBOUNCE_THRESHOLD, this);
    ...
    await sleep(2000);
    expect(protocol).toEqual(["await-3", "await-5", "await-7", "run-7"]);
  });

  it.skip("High volume calls", async () => {
