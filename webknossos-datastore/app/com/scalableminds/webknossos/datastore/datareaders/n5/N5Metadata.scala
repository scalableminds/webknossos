...
case class N5Metadata(multiscales: List[N5MultiscalesItem]) derives JsonAutoFormat

case class N5MultiscalesItem(datasets: List[N5MultiscalesDataset]) derives JsonAutoFormat

case class N5MultiscalesDataset(path: String, transform: N5Transform) derives JsonAutoFormat

case class N5Transform(axes: List[String], scale: List[Double], units: Option[List[String]]) derives JsonAutoFormat
