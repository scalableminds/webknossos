...
class AnnotationService @Inject()(annotationInformationProvider: AnnotationInformationProvider,
  ...
  def createAnnotationFor(user: User, task: Task, initializingAnnotationId: ObjectId)(implicit m: MessagesProvider, ctx: DBAccessContext): Fox[Annotation] = {
  ...
  def zipAnnotations(annotations: List[Annotation], zipFileName: String)(implicit m: MessagesProvider, ctx: DBAccessContext): Fox[TemporaryFile] = {
    for {
