...
class TSAnnotationService @Inject() (
  ...
  def get(annotationId: String, version: Option[Long])(implicit
      ec: ExecutionContext,
      tc: TokenContext
  ): Fox[AnnotationProto] =
    for {
  ...
  )(implicit ec: ExecutionContext, tc: TokenContext): Fox[Seq[AnnotationProto]] =
    Fox.serialCombined(annotationIds) { annotationId =>
      get(annotationId, None)
    }

  private def getWithTracings(annotationId: String, version: Option[Long])(implicit
      ec: ExecutionContext,
      tc: TokenContext
    ...
    for {
      ...
      newestMaterialized <- getNewestMatchingMaterializedAnnotation(
        annotationId,
        version = None
      ) ?~> "getNewestMaterialized.failed"
      targetVersion <- determineTargetVersion(
        annotationId,
        newestMaterialized,
        version
      ) ?~> "determineTargetVersion.failed"
  ...
  private def getWithTracingsVersioned(annotationId: String, targetVersion: Long, reportChangesToWk: Boolean)(implicit
      ec: ExecutionContext,
      tc: TokenContext
  ): Fox[AnnotationWithTracings] =
    for {
      ...
      case a: UpdateMappingNameVolumeAction if a.isEditable.contains(true) =>
        for {
          withNewEditableMapping <- addEditableMapping(
            ...
            a,
            targetVersion
          ) ?~> "applyUpdate.addEditableMapping.failed"
  ...
  private def addLayer(
    ...
    for {
      ...
      ) ?~> "addLayer.onlyOneSkeletonAllowed"
  ...
  private def revertToVersion(
    ...
    for {
      ...
      _ <- revertDistributedElements(
        ...
        newVersion
      ) ?~> "revert.revertDistributedElements.failed"
      _ = Instant.logSince(
        before,
  ...
  def updateActionLog(annotationId: String, newestVersion: Long, oldestVersion: Long)(implicit
      ec: ExecutionContext
  ): Fox[JsValue] = {
  ...
  def findEditableMappingInfo(annotationId: String, tracingId: String, version: Option[Long] = None)(implicit
      ec: ExecutionContext,
      tc: TokenContext
  ...
  private def addEditableMapping(
      ...
      targetVersion: Long
  )(implicit tc: TokenContext, ec: ExecutionContext): Fox[AnnotationWithTracings] =
    for {
      ...
      ) ?~> "annotation.volumeBucketsNotEmpty"
  ...
  private def findEditableMappingsForAnnotation(
      ...
      targetVersion: Long
  )(implicit ec: ExecutionContext, tc: TokenContext) = {
    ...
    for {
      idInfoUpdaterTuples <- Fox.serialCombined(volumeWithEditableMapping) { case (volumeTracing, volumeTracingId) =>
        for {
          ...
          ) ?~> "getEditableMappingInfoRaw.failed"
          updater <- editableMappingUpdaterFor(
            annotationId,
            volumeTracingId,
            volumeTracing,
  ...
  private def editableMappingUpdaterFor(
      ...
      currentMaterializedVersion: Long,
      targetVersion: Long
  )(implicit tc: TokenContext): EditableMappingUpdater =
  ...
  protected def editableMappingUpdaterFor(
      ...
      currentMaterializedVersion: Long,
      targetVersion: Long
  )(implicit tc: TokenContext, ec: ExecutionContext): Fox[EditableMappingUpdater] =
    for {
  ...
  def editableMappingLayer(annotationId: String, tracingId: String, tracing: VolumeTracing)(implicit
      tc: TokenContext
  ): EditableMappingLayer =
    EditableMappingLayer(
      tracingId,
  ...
  def baseMappingName(annotationId: String, tracingId: String, tracing: VolumeTracing)(implicit
      ec: ExecutionContext,
      tc: TokenContext
  ): Fox[Option[String]] =
  ...
  def findVolume(annotationId: String, tracingId: String, version: Option[Long] = None)(implicit
      tc: TokenContext,
      ec: ExecutionContext
  ): Fox[VolumeTracing] =
    for {
  ...
  )(implicit tc: TokenContext, ec: ExecutionContext): Fox[List[Option[VolumeTracing]]] =
    Fox.combined {
      selectors.map {
        case Some(selector) =>
          for {
  ...
  def findMultipleSkeletons(
      selectors: Seq[Option[TracingSelector]]
  )(implicit tc: TokenContext, ec: ExecutionContext): Fox[List[Option[SkeletonTracing]]] =
    Fox.combined {
      selectors.map {
        case Some(selector) =>
          for {
  ...
  )(implicit ec: ExecutionContext, tc: TokenContext): Fox[AnnotationProto] =
    for {
                ...
                updateListAdapted <- Fox.serialCombined(updateList) {
                  case a: AddLayerAnnotationAction =>
                    for {
                      ...
                      mappedTracingId <- tracingIdMapMutable.get(
                        actionTracingId
                      ) ?~> s"Trying to duplicate addLayer update action v$version for unknown layer $actionTracingId. Current layer map: ${tracingIdMapMutable.toMap}"
                    } yield a.copy(tracingId = Some(mappedTracingId))
                  case a: LayerUpdateAction =>
                    for {
                      mappedTracingId <- tracingIdMapMutable.get(
                        a.actionTracingId
                      ) ?~> s"Trying to duplicate layer update action v$version for unknown layer ${a.actionTracingId}. Current layer map: ${tracingIdMapMutable.toMap}, v0TracingIds: $v0TracingIds"
  ...
  private def duplicateLayer(
      ...
      isFromTask: Boolean,
      datasetBoundingBox: Option[BoundingBox]
  )(implicit ec: ExecutionContext, tc: TokenContext): Fox[AnnotationLayerProto] =
    for {
  ...
  )(implicit ec: ExecutionContext, tc: TokenContext): Fox[String] =
    for {
