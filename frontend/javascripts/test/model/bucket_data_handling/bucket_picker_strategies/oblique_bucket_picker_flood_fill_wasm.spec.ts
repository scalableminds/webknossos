...
// Verifies that the WASM port (oblique_bucket_picker_flood_fill_wasm.ts /
// oblique_bucket_picker_flood_fill.c) produces exactly the same buckets (and priorities) as
// the original TypeScript flood-fill picker it's a port of, across a range of
...
async function collect(
  determineBucketsForPlane:
    | typeof determineBucketsForPlaneWithFloodFill
  ...
  // The wasm picker's determineBucketsForPlane is async; awaiting the JS flood-fill picker's
  // plain (non-promise) return value is a no-op.
...
describe("oblique_bucket_picker_flood_fill_wasm", () => {
  for (const scenario of SCENARIOS) {
    it(`matches the JS flood-fill picker for: ${scenario.name}`, async () => {
      const jsResults = await collect(determineBucketsForPlaneWithFloodFill, scenario);
      const wasmResults = await collect(determineBucketsForPlaneWithFloodFillWasm, scenario);
