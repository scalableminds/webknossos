    ...
    new AnnotationRestrictions {
      override def allowAccess(user: Option[User]) = {
        annotation.isPublic || user.exists {
          user =>
            annotation._user.contains(user._id) || user.isTeamManagerOf(annotation._team)
        }
      }
      ...
      override def allowFinish(user: Option[User]) = {
        user.exists {
          user =>
            (annotation._user.contains(user._id) || user.isTeamManagerOf(annotation._team)) && !(annotation.state == Finished)
        }
      }
    }
