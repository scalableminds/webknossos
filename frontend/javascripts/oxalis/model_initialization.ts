...
> {
	...
	if (initialCommandType.type === ControlModeEnum.TRACE) {
		...
		if (initialMaybeCompoundType != null) {
			annotation = await getAnnotationCompoundInformation(
				annotationId,
				initialMaybeCompoundType,
			);
		} else {
			let unversionedAnnotation =
			...
			const layersWithStats = annotationProto.annotationLayers.map(
				(protoLayer) => {
					return {
	...
	const serverVolumeTracingIds = serverVolumeTracings.map(
	...
	const annotationSpecificDatasetSettings =
		applyAnnotationSpecificViewConfiguration(
			annotation,
			dataset,
	...
	if (initialFetch) {
		...
		if (serverTracings.length > 0)
			Store.dispatch(
				setZoomStepAction(getSomeServerTracing(serverTracings).zoomLevel),
			);
		const { smallestCommonBucketCapacity, maximumLayerCountToRender } =
...
function maybeWarnAboutUnsupportedLayers(layers: Array<APIDataLayer>): void {
	for (const layer of layers) {
		if (!isElementClassSupported(layer)) {
			Toast.warning(
				messages["dataset.unsupported_element_class"](
					layer.name,
					layer.elementClass,
				),
				{
...
function initializeAnnotation(
	...
	if (controlMode !== ControlModeEnum.VIEW) {
		...
		if (skeletonTracing != null) {
			initializationActions.push(
				initializeSkeletonTracingAction(skeletonTracing),
			);
		}

		Store.dispatch(
			batchedAnnotationInitializationAction(initializationActions),
		);
...
function initializeDataset(
	...
	if (volumeTracings.length > 0) {
		const newDataLayers = getMergedDataLayersFromDatasetAndVolumeTracings(
			dataset,
			volumeTracings,
		);
...
function initializeAdditionalCoordinates(mutableDataset: MutableAPIDataset) {
	const unifiedAdditionalCoordinates =
		getUnifiedAdditionalCoordinates(mutableDataset);
	const initialAdditionalCoordinates = Utils.values(
...
function getMergedDataLayersFromDatasetAndVolumeTracings(
	...
	for (const tracing of tracings) {
		...
		const fallbackLayer =
			fallbackLayerIndex > -1 ? originalLayers[fallbackLayerIndex] : null;
		...
		if (allLayersSameRotation) {
			coordinateTransformsMaybe = {
				coordinateTransformations:
					originalLayers?.[0].coordinateTransformations,
			};
		...
		const tracingLayer: APISegmentationLayer = {
			...
			mappings:
				fallbackLayer != null && "mappings" in fallbackLayer
					? fallbackLayer.mappings
					: undefined,
			...
			additionalAxes: convertServerAdditionalAxesToFrontEnd(
...
function validateVolumeLayers(
	...
	const layersForVolumeTracings = volumeTracings.map((volumeTracing) =>
		dataLayers.find(
			(layer) =>
				layer.category === "segmentation" &&
				layer.tracingId === volumeTracing.id,
		),
	);
...
function determineDefaultState(
	...
	const { dataset, datasetConfiguration, temporaryConfiguration } =
		Store.getState();
	...
	const someTracing =
		tracings.length > 0 ? getSomeServerTracing(tracings) : null;
...
async function applyLayerState(stateByLayer: UrlStateByLayer) {
	for (const layerName of Object.keys(stateByLayer)) {
		...
		if (layerState.mappingInfo != null) {
			const { mappingName, mappingType, agglomerateIdsToImport } =
				layerState.mappingInfo;
			Store.dispatch(
			...
			if (agglomerateIdsToImport != null) {
				...
				if (tracing.skeleton == null) {
					Toast.error(
						messages["tracing.agglomerate_skeleton.no_skeleton_tracing"],
					);
					continue;
				}
				...
				if (mappingType !== "HDF5") {
					Toast.error(
						messages["tracing.agglomerate_skeleton.no_agglomerate_file_active"],
					);
					continue;
				}
		...
		if (layerState.meshInfo) {
			...
			if (currentMeshFileName != null) {
				const segmentationLayer = getSegmentationLayerByName(
					dataset,
					effectiveLayerName,
				);
				...
				await dispatchMaybeFetchMeshFilesAsync(
					Store.dispatch,
					segmentationLayer,
					dataset,
					false,
				...
				Store.dispatch(
					updateCurrentMeshFileAction(effectiveLayerName, currentMeshFileName),
				);
			}
		...
		if (layerState.connectomeInfo != null) {
			const { connectomeName, agglomerateIdsToImport } =
				layerState.connectomeInfo;
			Store.dispatch(
				updateCurrentConnectomeFileAction(effectiveLayerName, connectomeName),
			);
...
function enforcePricingRestrictionsOnUserConfiguration(
	...
	if (
		!isFeatureAllowedByPricingPlan(activeOrganization, PricingPlanEnum.Team)
	) {
		return {
			...userConfiguration,
...
function applyAnnotationSpecificViewConfiguration(
	...
	const initialDatasetSettings: Mutable<DatasetConfiguration> = _.cloneDeep(
		originalDatasetSettings,
	);
