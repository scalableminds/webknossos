...
import com.scalableminds.webknossos.datastore.helpers.NodeDefaults.createdTimestamp
...
class VolumeTracingService @Inject() (
  ...
  private def updateBucket(
    ...
    for {
      ...
      ) ?~> s"Received a bucket at negative position (${action.bucketPosition}), must be positive"
      _ <- assertMagIsValid(
        volumeLayer.tracing,
        action.mag
          ...
          if (magSets.isEmpty)
            ...
            if (!magsDoMatch)
              ...
              for {
                ...
                _ <- Fox.fromBool(
                  ElementClass.largestSegmentIdIsInRange(mergedVolume.largestSegmentId, tracing.elementClass)
                ) ?~> Messages(
  ...
  )(implicit ec: ExecutionContext, tc: TokenContext): Fox[Path] = {
  ...
  def findData(annotationId: ObjectId, tracingId: String, tracing: VolumeTracing)(implicit
      tc: TokenContext
  ): Fox[Option[Vec3Int]] =
    for {
      _ <- Fox.successful(())
  ...
  private def mergeTwo(
    ...
    for {
      ...
      tracingBSegments =
        if (segmentIdMapB.isEmpty) tracingB.segments
        else {
  ...
  def mergeVolumeData(
    ...
    if (magSets.isEmpty)
      ...
      for {
        _ <- Fox.fromBool(
          ElementClass.largestSegmentIdIsInRange(mergedVolume.largestSegmentId, elementClassProto)
        ) ?~> Messages(
          "annotation.volume.largestSegmentIdExceedsRange",
          mergedVolume.largestSegmentId,
          elementClassProto
  ...
  def importVolumeData(
      ...
      zipFile: File,
      currentVersion: Int
  )(implicit mp: MessagesProvider, tc: TokenContext): Fox[Long] =
  ...
  def getFallbackLayer(annotationId: ObjectId, tracing: VolumeTracing)(implicit
      tc: TokenContext
  ): Fox[Option[RemoteFallbackLayer]] =
    fallbackLayerCache.getOrLoad(
      (annotationId, tracing.fallbackLayer, tc.userTokenOpt),
      t => getFallbackLayerFromWebknossos(t._1, t._2)
    )

  private def getFallbackLayerFromWebknossos(annotationId: ObjectId, fallbackLayerName: Option[String])(implicit
      tc: TokenContext
  ): Fox[Option[RemoteFallbackLayer]] =
    for {
