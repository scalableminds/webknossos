...
trait NgffExplorationUtils {
  ...
  private def selectAxisUnit(axes: List[NgffAxis], axisOrder: AxisOrder)(implicit
      ec: ExecutionContext
  ): Fox[LengthUnit] =
    for {
  ...
  protected def getAdditionalAxes(multiscale: NgffMultiscalesItem, remotePath: VaultPath)(using
    ...
    for {
      ...
      ) ?~> s"Additional axes names (${duplicateNames.mkString("", ", ", "")}) are not unique."
    } yield axes
  }

  protected def getNgffMultiscaleChannelCount(multiscale: NgffMultiscalesItem, remotePath: VaultPath)(using
      ec: ExecutionContext,
      tc: TokenContext
  ): Fox[Int] =
    for {
  ...
  )(using ec: ExecutionContext, tc: TokenContext): Fox[List[(StaticLayer, VoxelSize)]] =
    for {
      axisOrder <- extractAxisOrder(
        multiscale.axes
      ) ?~> "Could not extract XYZ axis order mapping. Does the data have x, y and z axes, stated in multiscales metadata?"
