...
function initTmpDB() {
  ...
  const dbName = execSync(scriptdir + "/db_name.sh", { env: { POSTGRES_URL: POSTGRES_URL } })
    .toString()
  ...
  const dbHost = execSync(scriptdir + "/db_host.sh", { env: { POSTGRES_URL: POSTGRES_URL } })
    .toString()
    .trim();
  return {
    dbName: dbName,
