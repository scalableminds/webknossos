...
class VolumeTracingService @Inject() (
  ...
  private def updateSegmentIndex(
    ...
    ) ?~> "volumeSegmentIndex.update.failed"
  ...
  private def updateBucket(
    ...
    for {
      ...
      _ <- bool2Fox(
        !bucketPosition.hasNegativeComponent
      ) ?~> s"Received a bucket at negative position ($bucketPosition), must be positive"
  ...
  def revertVolumeData(
    ...
    for {
      ...
      _ <- Fox.serialCombined(bucketStreamBeforeRevert) { case (bucketPosition, dataBeforeRevert, version) =>
  ...
  def initializeWithDataMultiple(tracingId: String, tracing: VolumeTracing, initialData: File)(implicit
      mp: MessagesProvider,
      tc: TokenContext
  ): Fox[Set[Vec3Int]] =
        ...
        mags <-
          ...
          if (magSets.isEmpty)
            ...
            if (!magsDoMatch)
              ...
              for {
                _ <- withZipsFromMultiZipAsync(initialData)((_, dataZip) =>
                  mergedVolume.addLabelSetFromDataZip(dataZip)
                )
                _ <- withZipsFromMultiZipAsync(initialData)((index, dataZip) =>
                  mergedVolume.addFromDataZip(index, dataZip)
                )
                _ <- bool2Fox(
                  ElementClass.largestSegmentIdIsInRange(mergedVolume.largestSegmentId.toLong, tracing.elementClass)
                ) ?~> Messages(
  ...
  )(implicit tc: TokenContext): Fox[Set[Vec3Int]] =
    if (tracing.version != 0L) {
      Failure("Tracing has already been edited.")
    } else {
  ...
  )(implicit ec: ExecutionContext, tc: TokenContext): Fox[Files.TemporaryFile] = {
  ...
  private def allDataToOutputStream(
      tracingId: String,
      tracing: VolumeTracing,
  ...
  def adaptVolumeForDuplicate(
      ...
      newVersion: Long
  )(implicit ec: ExecutionContext, tc: TokenContext): Fox[VolumeTracing] = {
  ...
  private def addBoundingBoxFromTaskIfRequired(
      tracing: VolumeTracing,
      isFromTask: Boolean,
      datasetBoundingBoxOpt: Option[BoundingBox]
  ): VolumeTracing =
  ...
  private def volumeTracingLayer(
      ...
      includeFallbackDataIfAvailable: Boolean = false
  )(implicit tc: TokenContext): VolumeTracingLayer =
    VolumeTracingLayer(
      name = tracingId,
  ...
  def updateMagList(tracingId: String, tracing: VolumeTracing, mags: Set[Vec3Int]): Fox[String] =
    for {
      ...
      id <- saveVolume(
        tracing.copy(mags = mags.toList.sortBy(_.maxDim).map(vec3IntToProto)),
        Some(tracingId),
        tracing.version
      )
    } yield id
  ...
  def createAdHocMesh(tracingId: String, tracing: VolumeTracing, request: WebknossosAdHocMeshRequest)(implicit
      tc: TokenContext
  ...
  def merge(
    ...
    def mergeTwoWithStats(
        tracingAWithIndex: Box[(VolumeTracing, Int)],
        tracingBWithIndex: Box[(VolumeTracing, Int)]
    ): Box[(VolumeTracing, Int)] =
      for {
  ...
  private def mergeTwo(
    ...
    val userBoundingBoxes = combineUserBoundingBoxes(
    ...
    for {
      ...
      boundingBox = mergedBoundingBox.getOrElse(
        com.scalableminds.webknossos.datastore.geometry
  ...
  private def bucketStreamFor(tracingId: String, tracing: VolumeTracing)(implicit
      tc: TokenContext
  ): Iterator[(BucketPosition, Array[Byte])] = {
    val dataLayer = volumeTracingLayer(tracingId, tracing)
    dataLayer.bucketProvider.bucketStream(Some(tracing.version))
  }
  ...
  def mergeVolumeData(
  ...
  )(implicit mp: MessagesProvider, tc: TokenContext): Fox[MergedVolumeStats] = {
    val before = Instant.now
    val elementClass = tracings.headOption.map(_.elementClass).getOrElse(elementClassToProto(ElementClass.uint8))

    val magSets = new mutable.HashSet[Set[Vec3Int]]()
    tracingIds.zip(tracings).foreach { case (tracingId, tracing) =>
      val magSet = new mutable.HashSet[Vec3Int]()
      bucketStreamFor(tracingId, tracing).foreach { case (bucketPosition, _) =>
    ...
    if (magSets.isEmpty)
      ...
      tracingIds.zip(tracings).zipWithIndex.foreach { case ((tracingIds, tracing), sourceVolumeIndex) =>
      ...
      for {
        _ <- bool2Fox(
          ElementClass.largestSegmentIdIsInRange(mergedVolume.largestSegmentId.toLong, elementClass)
        ) ?~> Messages(
          "annotation.volume.largestSegmentIdExceedsRange",
          mergedVolume.largestSegmentId.toLong,
          elementClass
        )
        mergedAdditionalAxes <- Fox.box2Fox(
          AdditionalAxis.mergeAndAssertSameAdditionalAxes(
            tracings.map(t => AdditionalAxis.fromProtosAsOpt(t.additionalAxes))
          )
        )
  ...
  def importVolumeData(tracingId: String, tracing: VolumeTracing, zipFile: File, currentVersion: Int)(implicit
      mp: MessagesProvider,
      tc: TokenContext
  ): Fox[Long] =
  ...
  def getFallbackLayer(tracingId: String, tracing: VolumeTracing)(implicit
      tc: TokenContext
  ): Fox[Option[RemoteFallbackLayer]] =
    fallbackLayerCache.getOrLoad(
      (tracingId, tracing.fallbackLayer, tc.userTokenOpt),
      t => getFallbackLayerFromWebknossos(t._1, t._2)
    )

  private def getFallbackLayerFromWebknossos(tracingId: String, fallbackLayerName: Option[String])(implicit
      tc: TokenContext
  ) =
    Fox[Option[RemoteFallbackLayer]] {
      for {
