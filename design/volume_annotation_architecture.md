...
- **Journal-backed undo/redo** (§5.7). The app still uses the existing bucket-snapshot undo stack. The journal is implemented and tested against its own unit tests, but is not on the live path, so the fold-from-base semantics are unexercised in the app. Two pieces of §5.7/§5.8 are missing inside it, both of which step 2 below has to supply:
  - **The undo horizon is not enforced.** §5.8's rule — the materialization point must stay at or behind the undo horizon — has no counterpart in `setBase`, which advances the base to whatever version was fetched. Since a fold only ever *adds* runs, a transaction that has been folded into the base can no longer be skipped out of it. `rebuild()` throws rather than returning stale content when it detects this, but the invariant that would prevent it needs the `undoTransaction` marker and the re-fetch path, neither of which exists yet.
  - **No local checkpoints.** §5.7 folds undo from a local checkpoint and a bucket load from fetched backend data; the implementation has a single `base` field serving both. Checkpoints only bound replay length, so this is not a correctness gap on its own — but they carry the same horizon rule, and the two roles should be separate fields once undo is real.
