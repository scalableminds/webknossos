...
- `Tree splitting` can be done in two ways:
    ...
    2. Delete an edge between two nodes. Select the first node (_Left Click_), then right-click the second node and select _Delete Edge to this Node_. If you have enabled _Classic Controls_, you need to select the first node with _Shift + Left Click_ and then click on the second node of the edge with _SHIFT + CTRL + Left Click_ to delete this connection.
