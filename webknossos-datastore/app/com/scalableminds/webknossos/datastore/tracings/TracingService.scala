...
trait TracingService[T <: GeneratedMessage with Message[T]] extends KeyValueStoreImplicits with FoxImplicits with LazyLogging {
  ...
  def handleUpdateGroup(tracingId: String, updateGroup: UpdateActionGroup[T])(implicit ec: ExecutionContext): Fox[_]

  def applyPendingUpdates(tracing: T, tracingId: String, targetVersion: Option[Long])(implicit ec: ExecutionContext): Fox[T] = Fox.successful(tracing)

  def find(tracingId: String, version: Option[Long] = None, useCache: Boolean = true, applyUpdates: Boolean = false)(implicit ec: ExecutionContext): Fox[T] = {
  ...
  def findMultiple(selectors: List[TracingSelector], useCache: Boolean = true, applyUpdates: Boolean = false)(implicit ec: ExecutionContext): Fox[List[T]] = {
    Fox.combined {
      selectors.map(selector => find(selector.tracingId, selector.version, useCache, applyUpdates))
    }
  }

  def save(tracing: T, tracingId: Option[String], version: Long, toCache: Boolean = false)(implicit ec: ExecutionContext): Fox[String] = {
