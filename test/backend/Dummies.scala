...
object Dummies {
  ...
  val tree2 = Tree(2, Seq(createDummyNode(4), createDummyNode(5), createDummyNode(6)),
    ...
    "TestTree-2", timestamp, Some(1))

  val treeGroup1 = TreeGroup("Axon 1", 1, Seq(TreeGroup("Blah", 3), TreeGroup("Blah 2", 4)))
  val treeGroup2 = TreeGroup("Axon 2", 2)

  val tracing = SkeletonTracing("dummy_dataset", Seq(tree1, tree2), timestamp, None, Some(1), Point3D(1, 1, 1),
    Vector3D(1.0, 1.0, 1.0), 1.0, 0, None, Seq(treeGroup1, treeGroup2))
  ...
  val componentTree = Tree(3, comp1Nodes ++ comp2Nodes, comp1Edges ++ comp2Edges,
    None, Seq(), Seq(), "Test Tree-3", timestamp, None)

  val emptyTree = Tree(4, Seq(), Seq(), None, Seq(), Seq(), "Test Tree-3", timestamp, None)

  val componentTracing = SkeletonTracing("dummy_dataset", Seq(componentTree, emptyTree), timestamp, None, None, Point3D(1, 1, 1),
    Vector3D(1.0, 1.0, 1.0), 1.0, 0, None, Seq())
}
